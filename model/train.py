...
DEFAULT_TOKENIZER_MODEL = Path(
    os.getenv("DEFAULT_TOKENIZER_MODEL", "sp_anecdote.model")
)
...
def load_tokenizer(
    model_path: Path | str = DEFAULT_TOKENIZER_MODEL,
) -> spm.SentencePieceProcessor:
    return spm.SentencePieceProcessor(model_file=str(model_path))


def tokenize(
    texts: list[str], model_path: Path | str = DEFAULT_TOKENIZER_MODEL
) -> list[list[int]]:
...
def iter_token_stream(
    tokenizer: spm.SentencePieceProcessor, texts: Iterable[str]
) -> list[int]:
...
def generate_sample(
    ...
    input_ids = torch.tensor(
        [tokenizer.encode(prompt, out_type=int)], dtype=torch.long, device=device
    )
